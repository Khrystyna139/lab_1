...
BEGIN
    ...
    -- Отримуємо вхідні дані для формули з вашої таблиці Books
    SELECT 
    ...
    -- АЛГОРИТМ РОЗРАХУНКУ ЦІННОСТІ:
    SET @Value = (@Rating * 2) + (@Pages / 100.0);
    IF (@Status = N'прочитана') SET @Value = @Value + 3;
    IF (@InLib = 1) SET @Value = @Value + 2;

    -- ЛОГІКА ОЧИЩЕННЯ СТАРИХ МАРКЕРІВ:
    -- Знаходимо позицію маркера, щоб при повторному запуску не дублювати його вмістом
    ...
    -- ЗАПИС результату назад у поле Note
    UPDATE Books
    SET Note = CASE 
...
-- ==========================================
-- 2. Процедура розрахунку цінності за вказаний період років
-- ==========================================
...
BEGIN
    ...
    -- Курсор для перебору книг за рік першого видання (FirstPubYear)
    ...
    PRINT N'Обробку за період завершено.';
END;
GO

-- ==========================================
-- 3. Процедура розрахунку цінності для ВСІХ книг
-- ==========================================
...
BEGIN
    ...
    -- Курсор для вибору абсолютно всіх книг у базі
...
-- ==========================================
-- ВИКОНАННЯ ТА ПЕРЕВІРКА РЕЗУЛЬТАТІВ
-- ==========================================

-- Варіант А: Оновити цінність для всіх книг, виданих з 2000 по 2026 рік (можете розкоментувати)
-- EXEC sp_BooksValueByPeriod 2000, 2026;
-- GO

-- Варіант Б: Оновити цінність для абсолютно всіх книг у базі
EXEC sp_CalculateAllBooksValue;
GO

-- Дивимось результат в таблиці
